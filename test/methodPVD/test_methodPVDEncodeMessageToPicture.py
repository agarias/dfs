...
class MethodPVDEncodeMessageToPictureTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        print('In setUpClass() MethodPVDEncodeMessageToPicture')
        cls.pvd = utils.import_module("pvd", "dfstego/methods/methodPVD.py")

    @classmethod
    def tearDownClass(cls):
        print('In tearDownClass() MethodPVDEncodeMessageToPicture')
        del cls.pvd
    ...
    def test_encode_message_to_picture_image_correct(self):
        check_result = 1 
        message = "a"
        self.assertEqual(self.pvd.encode_message_to_picture(self.name_image,message),check_result)
        check_result = 0
        im = utils.read_image(self.stego_image)
        self.assertNotEqual(im,check_result)
        result = self.pvd.decode_message_to_picture(self.stego_image)
