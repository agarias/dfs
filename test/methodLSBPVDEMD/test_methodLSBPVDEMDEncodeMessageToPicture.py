...
class MethodLSBPVDEMDEncodeMessageToPictureTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        print('In setUpClass() MethodLSBPVDEMDEncodeMessageToPicture')
        cls.merge = utils.import_module("merge", "dfstego/methods/methodLSBPVDEMD.py")

    @classmethod
    def tearDownClass(cls):
        print('In tearDownClass() MethodLSBPVDEMDEncodeMessageToPicture')
        del cls.merge
    ...
    def test_encode_message_to_picture_image_correct(self):
        check_result = 1 
        message = "a"
        self.assertEqual(self.merge.encode_message_to_picture(self.name_image,message),check_result)
        check_result = 0
        im = utils.read_image(self.stego_image)
        self.assertNotEqual(im,check_result)
        result = self.merge.decode_message_to_picture(self.stego_image)
