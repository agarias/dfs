...
class MethodLSBEncodeMessageToPictureTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        print('In setUpClass() MethodLSBEncodeMessageToPictureTest')
        cls.elias = utils.import_module("elias", "dfstego/methods/methodLSB.py")

    @classmethod
    def tearDownClass(cls):
        print('In tearDownClass() MethodLSBEncodeMessageToPictureTest')
        del cls.elias
    ...
    def test_encode_message_to_picture_image_correct(self):
        check_result = 1 
        message = "a"
        self.assertEqual(self.elias.encode_message_to_picture(self.name_image,message),check_result)
        check_result = 0
        im = utils.read_image(self.stego_image)
        self.assertNotEqual(im,check_result)
        result = self.elias.decode_message_to_picture(self.stego_image)
